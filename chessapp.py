...
class chessapp:
    def __init__(self, root, ai):
        ...
        self.draw_board()
        self.move_sound = pygame.mixer.Sound('ChessAiApp\\sounds\\move-self.mp3')
    ...
    def animate_piece_movement(self, from_square, to_square):
        ...
        label = tk.Label(self.root, image=image,
                        bg=('brown' if (from_row + from_col) % 2 == 0 else 'white'))
        label.image = image

        label.grid(row=from_row, column=from_col)

        def animate_frame(frame):
            current_row = int((from_row * (1 - frame)) + (to_row * frame))
            current_col = int((from_col * (1 - frame)) + (to_col * frame))

            label.grid(row=current_row, column=current_col)
